#ifndef LAB_3SEM_3_SHRD_PTR_H
#define LAB_3SEM_3_SHRD_PTR_H
...
public:
	...
	ShrdPtr<T>& operator=(ShrdPtr<T> setted_ptr) noexcept;
	T& operator*() const noexcept;
	T* operator->() const noexcept;
	bool operator==(ShrdPtr<T> other_ptr) const noexcept;
};
...
template <typename T>
inline ShrdPtr<T>& ShrdPtr<T>::operator=(ShrdPtr<T> setted_ptr) noexcept {
	if (*counter_ == 1) {
	 	delete ptr_;
		delete counter_;
	} else
...
template <typename T>
bool ShrdPtr<T>::operator==(ShrdPtr<T> other_ptr) const noexcept {
