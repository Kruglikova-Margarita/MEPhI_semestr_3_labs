...
public:
    ...
    virtual ShrdPtr<Sequence<T>> getSubSequence(int start_index, int end_index) const = 0;
    ...
    virtual ShrdPtr<Sequence<T>> concat(const Sequence<T>& added_sequence) const noexcept = 0;
    virtual T operator[](int index) const = 0;
    virtual ShrdPtr<Sequence<T>> operator+(const Sequence<T>& added_sequence) const noexcept = 0;
...
public:
    ...
    ArraySequence(ShrdPtr<T[]> items, int count);
    ...
    ShrdPtr<Sequence<T>> getSubSequence(int start_index, int end_index) const override;
    ...
    ShrdPtr<Sequence<T>> concat(const Sequence<T>& added_sequence) const noexcept override;
    T operator[](int index) const override;
    ShrdPtr<Sequence<T>> operator+(const Sequence<T>& added_sequence) const noexcept override;
...
template <typename T>
ArraySequence<T>::ArraySequence(ShrdPtr<T[]> items, int count) {
    try {
        dynamic_array_ = UnqPtr<DynamicArray<T>>(new DynamicArray<T>(items, count));
    }
    catch (...) {
...
template <typename T>
ShrdPtr<Sequence<T>> ArraySequence<T>::getSubSequence(int start_index, int end_index) const {
    ...
    ShrdPtr<T[]> items(new T[end_index - start_index + 1]);
    
    for (int i = start_index; i <= end_index; i++) {
        items[i - start_index] = dynamic_array_->get(i);
    }

    ShrdPtr<Sequence<T>> sub_sequence = ShrdPtr<Sequence<T>>(new ArraySequence<T>(items, (end_index - start_index + 1)));
...
template <typename T>
ShrdPtr<Sequence<T>> ArraySequence<T>::concat(const Sequence<T>& added_sequence) const noexcept {
    size_t this_len = dynamic_array_->getLength();
    size_t added_len = added_sequence.getLength();

    ShrdPtr<Sequence<T>> concat_sequence(new ArraySequence<T>);
...
template <typename T>
ShrdPtr<Sequence<T>> ArraySequence<T>::operator+(const Sequence<T> &added_sequence) const noexcept {
    ShrdPtr<Sequence<T>> new_sequence = this->concat(added_sequence);
...
UnqPtr<Sequence<T>> ArraySequence<T>::map(T (*function)(T)) const noexcept {
    ...
    ShrdPtr<T[]> items = ShrdPtr<T[]>(new T[len]);
...
UnqPtr<Sequence<T>> ArraySequence<T>::where(bool (*function)(T)) const noexcept {
    ...
    ShrdPtr<T[]> items = ShrdPtr<T[]>(new T[count]);
    size_t index = 0;
...
template <typename T>
ShrdPtr<Sequence<T>> LinkedListSequence<T>::getSubSequence(int start_index, int end_index) const {
    ...
    ShrdPtr<Sequence<T>> subSequence = ShrdPtr<Sequence<T>>(new LinkedListSequence<T>(items, (end_index - start_index + 1)));
...
template <typename T>
ShrdPtr<Sequence<T>> LinkedListSequence<T>::concat(const Sequence<T>& added_sequence) const noexcept {
    size_t this_len = linked_list_->getLength();
    size_t added_len = added_sequence.getLength();

    if ((this_len + added_len) == 0) {
        ShrdPtr<Sequence<T>> concat_sequence = UnqPtr<Sequence<T>>(new LinkedListSequence<T>);
    ...
    ShrdPtr<Sequence<T>> concat_sequence = UnqPtr<Sequence<T>>(new LinkedListSequence<T>(items, this_len + added_len));
...
template <typename T>
ShrdPtr<Sequence<T>> LinkedListSequence<T>::operator+(const Sequence<T>& added_sequence) const noexcept {
    ShrdPtr<Sequence<T>> new_sequence = this->concat(added_sequence);
