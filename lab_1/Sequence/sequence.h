...
T ArraySequence<T>::getFirst() const {
    Option<T> first = dynamic_array_->getFirst();
...
T ArraySequence<T>::getLast() const {
    Option<T> last = dynamic_array_->getLast();
...
T ArraySequence<T>::get(int index) const {
    Option<T> item = dynamic_array_->get(index);
...
UnqPtr<Sequence<T>> ArraySequence<T>::getSubSequence(int start_index, int end_index) const {
    ...
    for (int i = start_index; i <= end_index; i++) {
        items[i - start_index] = (dynamic_array_->get(i)).get();
    }
...
UnqPtr<Sequence<T>> ArraySequence<T>::concat(const Sequence<T>& added_sequence) const noexcept {
    ...
    UnqPtr<T[]> items(new T[this_len + added_len]);

    for (int i = 0; i < this_len; i++) {
        items[i] = (dynamic_array_->get(i)).get();
    }
    ...
    UnqPtr<Sequence<T>> concat_sequence(new ArraySequence<T>(items, this_len + added_len));
...
UnqPtr<Sequence<T>> ArraySequence<T>::operator+(const Sequence<T> &added_sequence) const noexcept {
    UnqPtr<Sequence<T>> new_sequence = UnqPtr(this->concat(added_sequence));
    return new_sequence;
}
